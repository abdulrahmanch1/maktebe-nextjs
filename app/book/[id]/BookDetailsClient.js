...
const BookDetailsClient = ({ initialBook }) => {
  ...
  const handleToggleFavorite = async () => {
    ...
    setBook(prevBook => ({ ...prevBook, favoritecount: currentlyLiked ? prevBook.favoritecount - 1 : prevBook.favoritecount + 1 }));
    try {
      // The context handles the API call and its own toast notifications.
      await toggleFavorite(initialBook.id);
    } catch (error) {
      console.error('خطأ في تحديث المفضلة:', error);
      // Rollback the optimistic UI update if the context API call fails.
      setBook(prevBook => ({ ...prevBook, favoritecount: currentlyLiked ? prevBook.favoritecount : prevBook.favoritecount }));
    } finally {
