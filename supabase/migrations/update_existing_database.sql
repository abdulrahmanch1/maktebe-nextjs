-- ============================================
-- UPDATE EXISTING DATABASE WITH RLS
-- مكتبة دار القرَاء - تحديث قاعدة البيانات الموجودة
-- ============================================
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name='reading_list' AND column_name='updated_at') THEN
    ALTER TABLE reading_list ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
  END IF;
END $$;
...
-- ============================================
-- 6. UPDATE SUGGESTED BOOKS TABLE
-- ============================================
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name='suggested_books' AND column_name='updated_at') THEN
    ALTER TABLE suggested_books ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
  END IF;
END $$;
