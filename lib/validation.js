...
export const validateBook = (data) => {
  ...
  if (!data.publishYear || isNaN(data.publishYear)) errors.publishYear = 'سنة التأليف يجب أن تكون رقمًا.';
...
export const validateBookUpdate = (data) => {
  const errors = {};
  if (data.title !== undefined && !data.title) errors.title = 'العنوان مطلوب.';
  if (data.author !== undefined && !data.author) errors.author = 'المؤلف مطلوب.';
  if (data.category !== undefined && !data.category) errors.category = 'التصنيف مطلوب.';
  if (data.description !== undefined && !data.description) errors.description = 'الوصف مطلوب.';
  if (data.pages !== undefined && isNaN(data.pages)) errors.pages = 'عدد الصفحات يجب أن يكون رقمًا.';
  if (data.publishYear !== undefined && isNaN(data.publishYear)) errors.publishYear = 'سنة التأليف يجب أن تكون رقمًا.';
  if (data.language !== undefined && !data.language) errors.language = 'اللغة مطلوبة.';
  return errors;
};
